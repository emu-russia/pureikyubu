...
static void put(char * mnem, ULONG mask, ULONG chkval, int iclass)
{
...
static void integer(char *mnem, char form, int dab, int hex, int s, int crfD, int L, int imm)
{
    char * ptr = o->operands;
...
static void cmp(char *l, char *i)
{
    ...
    integer(mnem, (*i == 'i') ? 'D' : 'X', DAB_A|DAB_B, 0, 1, (rd >> 2) ? 1 : 0, 0, 1);
    o->iclass |= PPC_DISA_SIMPLIFIED;
#else
...
static void addi(char *suffix)
{
    ...
    {
        integer("li", 'D', DAB_D, 0, 1, 0, 0, 1);
        o->iclass |= PPC_DISA_SIMPLIFIED;
        return;
    }
    if( (suffix[0] == 's') && (DIS_RA == 0) )   // Load address HI
    {
        integer("lis", 'D', DAB_D, 1, 0, 0, 0, 1);
        o->iclass |= PPC_DISA_SIMPLIFIED;
        return;
    }
    if(DIS_UIMM & 0x8000)
    {
        ...
        // Fix immediate field.
        value = (USHORT)(~(DIS_UIMM) + 1);
        Instr = (Instr & ~0xFFFF) | value;

        integer(mnem, 'D', DAB_D|DAB_A, 0, 1, 0, 0, 1);
        o->iclass |= PPC_DISA_SIMPLIFIED;
    }
    else
    {
        sprintf(mnem, "addi%s", suffix);
        integer(mnem, 'D', DAB_D|DAB_A, 0, 0, 0, 0, 1);
    }
#else
    sprintf(mnem, "addi%s", suffix);
...
{
    char *old;
    ULONG *t = (ULONG *)&o->target;

    if(comma) ptr += sprintf(ptr, "%s", COMMA);
    old = ptr;
#ifdef  POWERPC_32
    ptr += sprintf(ptr, HEX1 "%08X" HEX2, (ULONG)o->target);
#endif
#ifdef  POWERPC_64
    ptr = old;
...
// CR logic operations
static void crop(char *name, char *simp, int ddd, int daa)
{
    int crfD, crfA, crfB;
...
#define MASK32(b, e) \
{ \
    ULONG mask = ((ULONG)0xffffffff >> (b)) ^ (((e) >= 31) ? 0 : ((ULONG)0xffffffff) >> ((e) + 1)); \
    o->target = ((b) > (e)) ? (~mask) : (mask); \
}

#define MASK64(b, e) \
{ \
    ULONGLONG mask = ((ULONGLONG)0xffffffffffffffff >> (b)) ^ (((e) >= 63) ? 0 : ((ULONGLONG)0xffffffffffffffff) >> ((e) + 1)); \
    o->target = ((b) > (e)) ? (~mask) : (mask); \
}

// Rotate left word.
static void rlw(char *name, int rb, int ins)
{
...
// Load/Store.
static void ldst(char *name, int x/*indexed*/, int load, int L, int string, int fload)
{
    SHORT imm;
    if(x) integer(name, fload ? 'F' : 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1);
    else
    {
...
// Cache.
static void cache(char *name, int flag)
{
    if (DIS_RD) { ill(); return; }
    else
    {
        integer(name, 'X', DAB_A|DAB_B, 0, 1, 0, 0, 1);
        o->r[0] = o->r[1];
        o->r[1] = o->r[2];
        o->r[2] = 0;
        o->iclass &= ~PPC_DISA_INTEGER;
...
static void fpu(char *name, ULONG mask, int type, int flag)
{
...
void PPCDisasm(PPCD_CB *discb)
{
    ...
    */
    ...
    switch(Instr >> 26 /* Main opcode, base 8 */) {
        ...
        case 007: integer("mulli", 'D', DAB_D|DAB_A, 0, 1, 0, 0, 1); break; // mulli
        case 010: integer("subfic", 'D', DAB_D|DAB_A, 0, 1, 0, 0, 1); break; // subfic
        ...
        case 021: put("sc", 0x03ffffff, 2, PPC_DISA_OTHER); break;          // sc
        case 022: bx(); break;                                              // bx
        case 024: rlw("imi", 0, 1); break;                                  // rlwimix
        case 025: rlw("inm", 0, 0); break;                                  // rlwinmx
        case 027: rlw("nm", 1, 0); break;                                   // rlwnmx
        case 030:                                                           // ori
#ifdef SIMPLIFIED
                  if(Instr == 0x60000000) put("nop", 0, 0, PPC_DISA_INTEGER | PPC_DISA_SIMPLIFIED);
                  else
#endif
                  integer("ori", 'S', ASB_A|ASB_S, 1, 0, 0, 0, 1); break;
        case 031: integer("oris", 'S', ASB_A|ASB_S, 1, 0, 0, 0, 1); break;  // oris
        case 032: integer("xori", 'S', ASB_A|ASB_S, 1, 0, 0, 0, 1); break;  // xori
        case 033: integer("xoris", 'S', ASB_A|ASB_S, 1, 0, 0, 0, 1); break; // xoris
        case 034: integer("andi.", 'S', ASB_A|ASB_S, 1, 0, 0, 0, 1); break; // andi.
        case 035: integer("andis.", 'S', ASB_A|ASB_S, 1, 0, 0, 0, 1); break;// andis.
        case 040: ldst("lwz", 0, 1, 0, 0, 0); break;                        // lwz
        case 041: ldst("lwzu", 0, 1, 0, 0, 0); break;                       // lwzu
        case 042: ldst("lbz", 0, 1, 0, 0, 0); break;                        // lbz
        case 043: ldst("lbzu", 0, 1, 0, 0, 0); break;                       // lbzu
        case 044: ldst("stw", 0, 0, 0, 0, 0); break;                        // stw
        case 045: ldst("stwu", 0, 0, 0, 0, 0); break;                       // stwu
        case 046: ldst("stb", 0, 0, 0, 0, 0); break;                        // stb
        case 047: ldst("stbu", 0, 0, 0, 0, 0); break;                       // stbu
        case 050: ldst("lhz", 0, 1, 0, 0, 0); break;                        // lhz
        case 051: ldst("lhzu", 0, 1, 0, 0, 0); break;                       // lhzu
        case 052: ldst("lha", 0, 1, 0, 0, 0); break;                        // lha
        case 053: ldst("lhau", 0, 1, 0, 0, 0); break;                       // lhau
        case 054: ldst("sth", 0, 0, 0, 0, 0); break;                        // sth
        case 055: ldst("sthu", 0, 0, 0, 0, 0); break;                       // sthu
        case 056: ldst("lmw", 0, 1, 0, 1, 0); break;                        // lmw
        case 057: ldst("stmw", 0, 0, 0, 1, 0); break;                       // stmw
    ...
    */
    ...
    switch((Instr >> 1) & 0x3ff /* Extended opcode 023, base 8 */) {
        ...
        case 00401: crop("and", "", 0, 0); break;                           // crand
        case 00201: crop("andc", "", 0, 0); break;                          // crandc
        case 00441: crop("eqv", "set", 1, 0); break;                        // creqv
        case 00341: crop("nand", "", 0, 0); break;                          // crnand
        case 00041: crop("nor", "not", 0, 1); break;                        // crnor
        case 00701: crop("or", "move", 0, 1); break;                        // cror
        case 00641: crop("orc", "", 0, 0); break;                           // crorc
        case 00301: crop("xor", "clr", 1, 0); break;                        // crxor
        case 00226: put("isync", 0x3fff801, 0, PPC_DISA_OTHER); break;      // isync
#ifdef  POWERPC_32
    ...
    */
    ...
    switch(Instr & 0x7ff /* Extended opcode 037, base 8 */) {
...
#endif
                    trap(0, 0); break;
        case 00020: integer("subfc", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;   // subfcx
        case 00020|OE: integer("subfco", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00021: integer("subfc.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00021|OE: integer("subfco.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00024: integer("addc", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;         // addcx
        case 00024|OE: integer("addco", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00025: integer("addc.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00025|OE: integer("addco.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00026: integer("mulhwu", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;       // mulhwu
        case 00027: integer("mulhwu.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00046: if(DIS_RA | DIS_RB) ill();                              // mfcr
                    else { integer("mfcr", 'D', DAB_D, 0,0,0,0,0); o->iclass = PPC_DISA_OTHER; } break;
        case 00050: ldst("lwarx", 1, 1, 0, 0, 0); break;                    // lwarx
        case 00056: ldst("lwzx", 1, 1, 0, 0, 0); break;                     // lwzx
        case 00060: integer("slw", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;          // slwx
        case 00061: integer("slw.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 00064: if(DIS_RB) ill();                                       // cntlzwx
                    else integer("cntlzw", 'S', ASB_A|ASB_S, 0,0,0,0,0); break;
        case 00065: if(DIS_RB) ill();
                    else integer("cntlzw.", 'S', ASB_A|ASB_S, 0,0,0,0,0); break;
        case 00070: integer("and", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;          // andx
        case 00071: integer("and.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 00100: cmp("l", ""); break;                                    // cmpl
        case 00120: integer("subf", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;         // subfx
        case 00120|OE: integer("subfo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00121: integer("subf.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00121|OE: integer("subfo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00154: cache("dcbst", PPC_DISA_OTHER); break;                                  // dcbst
        case 00156: ldst("lwzux", 1, 1, 0, 0, 0); break;                    // lwzux
        case 00170: integer("andc", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;         // andcx
        case 00171: integer("andc.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 00226: integer("mulhw", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;        // mulhw
        case 00227: integer("mulhw.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00246: if(DIS_RA || DIS_RB) ill();                             // mfmsr
                    else { integer("mfmsr", 'X', DAB_D, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_OEA; } break;
        case 00254: cache("dcbf", PPC_DISA_OTHER); break;                                   // dcbf
        case 00256: ldst("lbzx", 1, 1, 0, 0, 0); break;                     // lbzx
        case 00320: if(DIS_RB) ill();                                       // negx
                    else integer("neg", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00321: if(DIS_RB) ill();
                    else integer("neg.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00320|OE: if(DIS_RB) ill();
                    else integer("nego", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00321|OE: if(DIS_RB) ill();
                    else integer("nego.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00356: ldst("lbzux", 1, 1, 0, 0, 0); break;                    // lbzux
        case 00370:                                                         // norx
#ifdef SIMPLIFIED
                    if(DIS_RS == DIS_RB) { integer("not", 'Z', ASB_A|ASB_S, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_SIMPLIFIED; }
                    else
#endif
                    integer("nor", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 00371: integer("nor.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 00420: integer("subfe", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;        // subfex
        case 00420|OE: integer("subfeo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00421: integer("subfe.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00421|OE: integer("subfeo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00424: integer("adde", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;         // addex
        case 00424|OE: integer("addeo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00425: integer("adde.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00425|OE: integer("addeo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00440: mtcrf(); break;                                         // mtcrf
#ifdef POWERPC_32
        case 00444: if(DIS_RA || DIS_RB) ill();                             // mtmsr
                    else { integer("mtmsr", 'X', DAB_D, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_OEA | PPC_DISA_BRIDGE; } break;
#endif
        case 00455: ldst("stwcx.", 1, 0, 0, 0, 0); break;                   // stwcx.
        case 00456: ldst("stwx", 1, 0, 0, 0, 0); break;                     // stwx
        case 00556: ldst("stwux", 1, 0, 0, 0, 0); break;                    // stwux
        case 00620: if(DIS_RB) ill();                                       // subfzex
                    else integer("subfze", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00620|OE: if(DIS_RB) ill();
                    else integer("subfzeo", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00621: if(DIS_RB) ill();
                    else integer("subfze.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00621|OE: if(DIS_RB) ill();
                    else integer("subfzeo.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00624: if(DIS_RB) ill();                                       // addzex
                    else integer("addze", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00624|OE: if(DIS_RB) ill();
                    else integer("addzeo", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
...
#endif
        case 00656: ldst("stbx", 1, 0, 0, 0, 0); break;                     // stbx
        case 00720: if(DIS_RB) ill();                                       // subfmex
                    else integer("subfme", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00720|OE: if(DIS_RB) ill();
                    else integer("subfmeo", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00721: if(DIS_RB) ill();
                    else integer("subfme.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00721|OE: if(DIS_RB) ill();
                    else integer("subfmeo.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00724: if(DIS_RB) ill();                                       // addmex
                    else integer("addme", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00724|OE: if(DIS_RB) ill();
                    else integer("addmeo", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00725: if(DIS_RB) ill();
                    else integer("addme.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00725|OE: if(DIS_RB) ill();
                    else integer("addmeo.", 'X', DAB_D|DAB_A, 0, 1, 0, 0, 1); break;
        case 00726: integer("mullw", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;        // mullwx
        case 00726|OE: integer("mullwo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00727: integer("mullw.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 00727|OE: integer("mullwo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
#ifdef POWERPC_32
        case 00744: movesr("mtsrin", 0, 0, 1); break;                       // mtsrin
#endif
        case 00754: cache("dcbtst", PPC_DISA_OTHER); break;                                 // dcbtst
        case 00756: ldst("stbux", 1, 0, 0, 0, 0); break;                    // stbux
        case 01024: integer("add", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;          // addx
        case 01024|OE: integer("addo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01025: integer("add.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01025|OE: integer("addo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01054: cache("dcbt", PPC_DISA_OTHER); break;                                   // dcbt
        case 01056: ldst("lhzx", 1, 1, 0, 0, 0); break;                     // lhzx
        case 01070: integer("eqv", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;          // eqvx
        case 01071: integer("eqv.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 01144: if(DIS_RD || DIS_RA) ill();                             // tlbie
                    else { integer("tlbie", 'X', DAB_B, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_OEA | PPC_DISA_OPTIONAL; o->r[0] = o->r[2]; o->r[2] = 0; } break;
        case 01154: integer("eciwx", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_OPTIONAL; break; // eciwx
        case 01156: ldst("lhzux", 1, 1, 0, 0, 0); break;                    // lhzux
        case 01170: integer("xor", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;          // xorx
        case 01171: integer("xor.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 01246: movespr(1); break;                                      // mfspr
        case 01256: ldst("lhax", 1, 1, 0, 0, 0); break;                     // lhax
#if !defined(GEKKO)
        case 01344: put("tlbia", 0x03FFF800, 0, PPC_DISA_OEA | PPC_DISA_OPTIONAL); break; // tlbia
#endif
        case 01346: movetbr(); break;                                       // mftb
        case 01356: ldst("lhaux", 1, 1, 0, 0, 0); break;                    // lhaux
        case 01456: ldst("sthx", 1, 0, 0, 0, 0); break;                     // sthx
        case 01470: integer("orc", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;          // orcx
        case 01471: integer("orc.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 01554: integer("ecowx", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_OPTIONAL; break; // ecowx
        case 01556: ldst("sthux", 1, 0, 0, 0, 0); break;                    // sthux
        case 01570:                                                         // orx
#ifdef SIMPLIFIED
                    if(DIS_RS == DIS_RB) { integer("mr", 'Z', ASB_A|ASB_S, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_SIMPLIFIED; }
                    else
#endif
                    integer("or", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 01571: 
#ifdef SIMPLIFIED
                    if(DIS_RS == DIS_RB) { integer("mr.", 'Z', ASB_A|ASB_S, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_SIMPLIFIED; }
                    else
#endif
                    integer("or.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 01626: integer("divwu", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;        // divwux
        case 01626|OE: integer("divwuo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01627: integer("divwu.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01627|OE: integer("divwuo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01646: movespr(0); break;                                      // mtspr
        case 01654: cache("dcbi", PPC_DISA_OEA); break;                     // dcbi
        case 01670: integer("nand", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;         // nandx
        case 01671: integer("nand.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 01726: integer("divw", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;         // divwx
        case 01726|OE: integer("divwo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01727: integer("divw.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 01727|OE: integer("divwo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); break;
        case 02000: mcrxr(); break;                                         // mcrxr
        case 02052: ldst("lswx", 1, 1, 0, 1, 0); break;                     // lswx
        case 02054: ldst("lwbrx", 1, 1, 0, 0, 0); break;                    // lwbrx
        case 02056: ldst("lfsx", 1, 1, 0, 0, 1); break;                     // lfsx
        case 02060: integer("srw", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;          // srwx
        case 02061: integer("srw.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        ...
        case 02254: put("sync", 0x03FFF800, 0, PPC_DISA_OTHER); break;      // sync
...
#endif
        case 02452: ldst("stswx", 1, 1, 0, 1, 0); break;                    // stswx
        case 02454: ldst("stwbrx", 1, 0, 0, 0, 0); break;                   // stwbrx
        ...
        case 03054: ldst("lhbrx", 1, 1, 0, 0, 0); break;                    // lhbrx
        case 03060: integer("sraw", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;         // srawx
        case 03061: integer("sraw.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); break;
        case 03160: srawi(); break;                                         // srawi
        case 03161: srawi(); break;
        case 03254: put("eieio", 0x03FFF800, 0, PPC_DISA_OTHER); break;     // eieio
        case 03454: ldst("sthbrx", 1, 0, 0, 0, 0); break;                   // sthbrx
        ...
        case 03654: cache("icbi", PPC_DISA_OTHER); break;                   // icbi
        case 03656: ldst("stfiwx", 1, 1, 0, 0, 1); o->iclass |= PPC_DISA_OPTIONAL; break; // stfiwx
        case 03754: cache("dcbz", PPC_DISA_OTHER); break;                   // dcbz
#ifdef POWERPC_64
        case 00022: integer("mulhdu", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // mulhdux
        case 00023: integer("mulhdu.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 00052: ldst("ldx", 1, 1, 1, 0, 0); break;                      // ldx
        case 00066: integer("sld", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // sldx
        case 00067: integer("sld.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 00152: ldst("ldux", 1, 1, 1, 0, 0); break;                     // ldux
        ...
        case 00222: integer("mulhd", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // mulhdx
        case 00223: integer("mulhd.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 00244: movesr("mtsrd", 0, 1, 0); break;                        // mtrsd
        case 00250: ldst("ldarx", 1, 1, 1, 0, 0); break;                    // ldarx
        case 00344: movesr("mtsrdin", 0, 1, 1); break;                      // mtrsdin
        case 00452: ldst("stdx", 1, 0, 1, 0, 0); break;                     // stdx
        case 00544: if(DIS_RA || DIS_RB) ill();                             // mtmsrd
                    else { integer("mtmsrd", 'X', DAB_D, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_OEA | PPC_DISA_64; } break;
        case 00552: ldst("stdux", 1, 0, 1, 0, 0); break;                    // stdux
        case 00655: ldst("stdcx.", 1, 0, 1, 0, 0); break;                   // stdcx.
        case 00722: integer("mulld", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // mulldx
        case 00722|OE: integer("mulldo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 00723: integer("mulld.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 00723|OE: integer("mulldo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01252: ldst("lwax", 1, 1, 1, 0, 0); break;                     // lwax
        case 01352: ldst("lwaux", 1, 1, 1, 0, 0); break;                    // lwaux
        ...
        case 01544: if(DIS_RD || DIS_RA) ill();                             // slbie
                    else { integer("slbie", 'X', DAB_B, 0, 1, 0, 0, 1); o->iclass = PPC_DISA_64 | PPC_DISA_OEA | PPC_DISA_OPTIONAL; o->r[0] = o->r[2]; o->r[2] = 0; } break;
        case 01622: integer("divdu", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // divdux
        case 01622|OE: integer("divduo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01623: integer("divdu.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01623|OE: integer("divduo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01722: integer("divd", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // divdx
        case 01722|OE: integer("divdo", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01723: integer("divd.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01723|OE: integer("divdo.", 'X', DAB_D|DAB_A|DAB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 01744: put("slbia", 0x03FFF800, 0, PPC_DISA_64 | PPC_DISA_OEA | PPC_DISA_OPTIONAL); break; // slbia
        case 02066: integer("srd", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // srdx
        case 02067: integer("srd.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
        case 03064: integer("srad", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break; // sradx
        case 03065: integer("srad.", 'Z', ASB_A|ASB_S|ASB_B, 0, 1, 0, 0, 1); o->iclass |= PPC_DISA_64; break;
    ...
    */
        ...
        case 072:
    switch(Instr & 3) {
        case 0: Instr &= ~3; ldst("ld", 0, 1, 1, 0, 0); break;              // ld
        case 1: Instr &= ~3; ldst("ldu", 0, 1, 1, 0, 0); break;             // ldu
        case 2: Instr &= ~3; ldst("lwa", 0, 1, 1, 0, 0); break;             // lwa
        default: ill(); break;
    } break;
#endif
    ...
    */
    ...
    #define MASK_C  (0x1F <<  6)
        case 073:
    switch(Instr & 0x3F) {
        case 044: fpu("fdivs", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;      // fdivsx
        case 045: fpu("fdivs.", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;
        case 050: fpu("fsubs", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;      // fsubsx
        case 051: fpu("fsubs.", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;
        case 052: fpu("fadds", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;      // faddsx
        case 053: fpu("fadds.", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;
#if !defined(GEKKO)
        ...
        case 062: fpu("fmuls", MASK_B, FPU_DAC, PPC_DISA_OTHER); break;      // fmulsx
        case 063: fpu("fmuls.", MASK_B, FPU_DAC, PPC_DISA_OTHER); break;
        case 070: fpu("fmsubs", 0, FPU_DACB, PPC_DISA_OTHER); break;         // fmsubsx
        case 071: fpu("fmsubs.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        case 072: fpu("fmadds", 0, FPU_DACB, PPC_DISA_OTHER); break;         // fmaddsx
        case 073: fpu("fmadds.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        case 074: fpu("fnmsubs", 0, FPU_DACB, PPC_DISA_OTHER); break;        // fnmsubsx
        case 075: fpu("fnmsubs.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        case 076: fpu("fnmadds", 0, FPU_DACB, PPC_DISA_OTHER); break;        // fnmaddsx
        case 077: fpu("fnmadds.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        default: ill(); break;
    } break;
    ...
    */
        ...
        case 076:
    switch(Instr & 3) {
        case 0: Instr &= ~3; ldst("std", 0, 0, 1, 0, 0); break;             // std
        case 1: Instr &= ~3; ldst("stdu", 0, 0, 1, 0, 0); break;            // stdu
        default: ill(); break;
    } break;
#endif
    ...
    */
    ...
    switch(Instr & 0x3F) {
        ...
        switch(DIS_RC)
        {
            case 18: fpu("mffs", MASK_A|MASK_B, FPU_D, PPC_DISA_OTHER); break;              // mffs
        ...
        switch(DIS_RC)
        {
            case 1: fpu("fneg", MASK_A, FPU_DB, PPC_DISA_OTHER); break;     // fneg
            case 2: fpu("fmr", MASK_A, FPU_DB, PPC_DISA_OTHER); break;      // fmr
            case 4: fpu("fnabs", MASK_A, FPU_DB, PPC_DISA_OTHER); break;    // fnabs
            case 8: fpu("fabs", MASK_A, FPU_DB, PPC_DISA_OTHER); break;     // fabs
            default: ill(); break;
        }
        break;
        case 021:
        ...
        switch(DIS_RC)
        {
            case 0: fpu("frsp", MASK_A, FPU_DB, PPC_DISA_OTHER); break;     // frsp
            default: ill(); break;
        }
        break;
        case 031:
        ...
        switch(DIS_RC)
        {
            case 0: fpu("fctiw", MASK_A, FPU_DB, PPC_DISA_OTHER); break;    // fctiw
#ifdef POWERPC_64
            case 25: fpu("fctid", MASK_A, FPU_DB, PPC_DISA_OTHER); break;   // fctid
            case 26: fpu("fcfid", MASK_A, FPU_DB, PPC_DISA_OTHER); break;   // fcfid
#endif
            default: ill(); break;
        }
        break;
        ...
        switch(DIS_RC)
        {
            case 0: fpu("fctiwz", MASK_A, FPU_DB, PPC_DISA_OTHER); break;   // fctiwz
#ifdef POWERPC_64
            case 25: fpu("fctidz", MASK_A, FPU_DB, PPC_DISA_OTHER); break;  // fctidz
#endif
            default: ill(); break;
        }
        break;
        ...
        break;
        case 044: fpu("fdiv", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;      // fdivx
        case 045: fpu("fdiv.", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;
        case 050: fpu("fsub", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;      // fsubx
        case 051: fpu("fsub.", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;
        case 052: fpu("fadd", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;      // faddx
        case 053: fpu("fadd.", MASK_C, FPU_DAB, PPC_DISA_OTHER); break;
#if !defined(GEKKO)
        ...
        case 062: fpu("fmul", MASK_B, FPU_DAC, PPC_DISA_OTHER); break;      // fmulx
        case 063: fpu("fmul.", MASK_B, FPU_DAC, PPC_DISA_OTHER); break;
        ...
        case 070: fpu("fmsub", 0, FPU_DACB, PPC_DISA_OTHER); break;         // fmsubx
        case 071: fpu("fmsub.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        case 072: fpu("fmadd", 0, FPU_DACB, PPC_DISA_OTHER); break;         // fmaddx
        case 073: fpu("fmadd.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        case 074: fpu("fnmsub", 0, FPU_DACB, PPC_DISA_OTHER); break;        // fnmsubx
        case 075: fpu("fnmsub.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        case 076: fpu("fnmadd", 0, FPU_DACB, PPC_DISA_OTHER); break;        // fnmaddx
        case 077: fpu("fnmadd.", 0, FPU_DACB, PPC_DISA_OTHER); break;
        default: ill(); break;
    } break;
