...
| `HleBios` | The high level BIOS calls against the official BIOS image, when the user has one: the SWI numbers, `HuffUnComp` byte for byte, `MidiKey2Freq` against the BIOS's own fixed point, and the sound driver's identifier, frequency table, FIFO DMA setup and VSync/VSyncOff (the image is not shipped, so those tests skip themselves without it) |
...
Every test passes: **332 of 332**. (The two that compare `build/Data/GBASettings.json` with
...
machine (5), the BIOS harness (2) and the HLE calls against the official BIOS (8).
