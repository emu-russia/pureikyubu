...
* the module's **debugging tools**: the ARM/Thumb and SM83 disassemblers of `src/gba/gba_disasm.cpp`
  ...
  machine that seems stuck. A bug report that arrives with a save state is reproduced with
  `--state <file>`, and the memory the machine was left in is read with `--peek <address> <words>`
  or written out raw with `--dump <address> <bytes> <file>` (the file `--disasm-arm`/`--disasm-thumb`
  then lists, which is how a routine the game copied into RAM is read).
...
check.sh --run game.gba --state game.st0 --frames 200        # run on from the user's save state
check.sh --run game.gba --state game.st0 --frames 200 --peek 0x02021E90 8
```
...
| `Bus` | The address decoder's subword rules: the byte lanes a wide access is assembled from, the display memory's byte-store quirks, the access cycles of the internal memories, and the byte lanes of the display registers (a byte store to DISPSTAT keeps the half it did not address) |
...
| `HleBios.HuffmanMatchesTheOfficialBios`, `HleBios.MidiKey2FreqMatchesTheOfficialBios`, `HleBios.TheSoundDriverMatchesTheOfficialBios` (the official BIOS image as the oracle) | `HuffUnComp` was a stub, so a game whose graphics are Huffman-compressed in the ROM could not boot without a real BIOS (Metroid Fusion was one: a black screen). It is implemented from GBATEK "SWI 13h" - and the differential test against the official BIOS found one thing the document leaves open: the hardware does **not** stop in the middle of a 32bit output unit, it keeps decoding until the unit that holds the last requested byte is full (with a 7 byte stream its last unit's fourth byte is a decoded symbol, not a zero). `MidiKey2Freq` was a double precision guess; it now uses the BIOS's own fixed point (a 16.16 semitone table, the octave as a shift, the product truncated, the fine value as a slope), which the oracle pins to within a unit per few thousand. The sound driver's entry points (`SoundDriverInit/Mode/ChannelClear/Main/VSync/VSyncOff/VSyncOn`) were stubs too; everything a probe could measure out of the real BIOS is implemented now: the work area's identifier 68736D53h and its 0FB0h byte size, the `pcmbuf` layout (two 0630h byte halves at +0350h and +0980h), the two FIFO DMAs (SAD at those halves, DAD at 40000A0h/40000A4h, CNT B600h - repeating with an incrementing source, which is what the DMA fix above is about), `SOUNDCNT_H` 210Eh, the mode fields in the header (+5 reverb, +6 the simultaneous channels, +7 the master volume) and the timer 0 reload for every playback frequency index. `SoundDriverMain` now mixes the twelve virtual channels into `pcmbuf` as well (GBATEK "SoundDriverMode" bits 8-11 give 1-12 channels, and the official mixer reads channel n at `50h + n * 40h`; the envelope pipeline, the volume and pan levels, and the phase accumulator, verified against the official driver by a differential test on every channel), so a game without a real BIOS has its music and its direct sound effects; what is still missing is the reverb (the mode bits are kept, but the driver's own delay line is not modelled) and the exact fixed point of the pitch stepping. |
| `Cart.EepromIdleReadDrivesTheReadyLine` (found by running The Legend of Zelda: The Minish Cap with a real `gba_bios.bin` and watching the save library's EEPROM traffic) | An idle read in the EEPROM's own window returned the ROM the window mirrors on **every** bit, including bit0 - which is not a data bit there at all: on an EEPROM cartridge bit0 of the ROM bus is the chip's **ready line**, and the games poll it after a write ("wait until bit0 is 1", GBATEK "GBA Cart Backup EEPROM"). `Cart::Reset` already set `eepromOutput = 1` "idle: bit0 high, the ready the games poll", but the read path never put it on the bus. Minish Cap's save library therefore saw a busy chip for the whole VCOUNT timeout on every write, gave up after three attempts and stamped `DAMEDAME` over the block it was writing (the failure marker of `0x7D20C` in its own library), so a fresh cartridge came up as "The data in File 1 is corrupted." and the file could not be started. With the ready line in place every write is verified on the first attempt and the file select shows the empty files.
| `HleBios.TheMixerReadsEveryChannelWhereTheOfficialDriverDoes` (found by putting a synthetic channel at each candidate offset and letting the official BIOS's own mixer say which one it reads) | The host-side sound driver's channel array was `16 entries of 30h bytes` (the count and stride were assumed to follow GBATEK's documented `vchn`), which spans the same 300h bytes as the real array and hides the mistake: only channel 0 landed where the official driver looks. The real array is **12 entries of 40h bytes** - GBATEK "SoundDriverMode" bits 8-11 say "1-12 channels", and the official mixer reads channel n at `50h + n * 40h`, exactly reaching `pcmbuf` at `350h`. With the 30h stride a game that calls the BIOS sound driver *without* a real image had channels 1..11 mixed from misaligned bytes (the next channel's envelope and volume bytes became this one's `fr` and wave pointer), which is heard as wrong notes, crackle and an apparent speed change. |
| `Dma.TheUnitSizeFollowsTheTransferTypeOnEveryChannel` (found while tracing The Minish Cap's sound engine, which moves its per-frame block with a 32bit DMA0) | `UsesWords` returned the 32bit unit size only for DMA3, so DMA0-2 always moved 16bit units whatever CNT_H bit 10 said. GBATEK's table gives bit 10 on every channel ("Specifies the number of data units to be transferred, each unit is 16bit or 32bit depending on the transfer type") - only bit 11 is marked "DMA3 only" - and the word count in the register is in *units*, so a channel left on 16bit units moves half of what the game asked for. The Minish Cap engine copies four 32bit words with DMA0 every other frame; the copy is now whole. |
| `Ppu.VCountMatchWriteDuringTheMatchDoesNotRequestItTwice` (found by tracing The Minish Cap's intro music, which played at exactly double speed while every other part of the game was correct) | `Ppu::UpdateVCountMatch` requested the V-Counter interrupt whenever the match was **true** on an evaluation, and a DISPSTAT write evaluates it too, so a program that wrote the register back while VCOUNT still equalled its setting took **two** match interrupts for one line. Minish Cap writes DISPSTAT at line 80 of every frame (the line it has just matched) and its engine - its own copy of the BIOS sound driver at `080B03xx`, called from the VBlank handler - advances its sequencer once per match interrupt, so the intro music ticked twice per frame: the note sequence advanced 12 bytes a second instead of 6, the notes came twice as fast, and the track ran out of sequence at 11 s and went silent while the game played on. The pitch stayed right, because the sample rate is timer 0 (15768 Hz) and that was untouched. The picture, the DMA/FIFO path (264 samples a frame, no underruns, the ring writes and the DMA source in step) and every non-sound byte of IWRAM and EWRAM were correct, so the engine was reading the same data and only its clock was wrong. The interrupt is now the edge of the gated (match AND enable) condition, which is what GBATEK 4000004h means by "requested when the flag becomes set"; a program that enables the interrupt while the counter already matches still gets it, which `Ppu.VCountMatchInterrupt` keeps covering. |
| `Bus, ByteWritesToTheDisplayRegistersKeepTheOtherLane` (found by running Castlevania: Circle of the Moon, which froze on its intro text - the red "It is 1830..." narration over the castle) | The bus's byte-lane rule for the display registers named the wrong register: `ReadIo8`/`WriteIo8` mirrored a byte into **both** lanes for offset 004h, which is DISPSTAT, while the rule and the comment are about the undocumented Green Swap at 04000002h. DISPSTAT's low byte carries the IRQ enables and its high byte the V-Count setting, and the game's raster engine writes each of them on its own: an 8 bit read-modify-write of 04000004h arms and disarms its V-Counter interrupt, an 8 bit store to 04000005h moves the comparison line. With the mirrored store the setting also landed in the enables, so the line the game asked for last - line 131, whose low byte 83h has bits 3-5 clear - took the V-Blank IRQ enable with it. The game's main loop waits for its V-Blank handler to tick a frame counter, so the narration stopped for ever where it stood: the counter froze 1105 frames into the state (the 6000-frame run left it at the same value as the 1500-frame one), the frame hashes repeated a seven-frame animation and then stopped changing at all, and the V-Blank raises stopped with the interrupt it had switched off. The mirroring now belongs to GREENSWP alone. |
| `Dma, AParkedHblankTransferCarriesOnWhereItStopped` (found by running Final Fantasy V Advance, whose message-window background flickered blue) | A start condition that arrives while another transfer is running waits for it - the running transfer lets the higher priority channels in at its next unit boundary (GBATEK "DMA Priority") - and the transfer it then runs is an ordinary repeat of the *waiting* channel, so it has to carry on from the source pointer that channel had reached. The unit-boundary path called `Perform` directly, which is the transfer without the bookkeeping `RunNow` does afterwards: the channel was left with its word count at zero, the next start condition took it for stale and re-latched its pointers from SAD and DAD, and a repeating block started over from its registers in the middle of the frame. Final Fantasy V Advance's message window is fed by an HBlank DMA of one palette entry per scanline whose source walks a gradient table (re-armed once a frame at VCOUNT=161, count 1, repeat, both pointers incrementing), and the parked request a long immediate DMA3 produces put the table's first entry into the middle of the window - with the other half of the bug, the parked request was also serviced a scanline late, because a request that arrived after the running transfer's last unit boundary stayed parked until the channel's *next* start condition. *Where* the restart happened moved from frame to frame with the immediate transfer's timing, so the gradient flickered between two shapes (with a real `gba_bios.bin`, `C9602E71` and `BFD645D6` alternating every frame); it is now the same picture every frame. `FinishTransfer` is the bookkeeping both paths share, and the requests that arrive after the last boundary are run as the transfer ends (`ServiceTriggered`). |

Everything else passes: the ARM7TDMI (67 tests), the Game Boy machine (95 - its CPU, LCD, cartridge,
boot ROM, the CGB WRAM/palette/HDMA bus paths and the sixteen APU tests), the LCD controller (44), the
sound (41: the GBA's 29 channel and mixer tests and the twelve of the mixer buffer between the
machine and the sound device), the settings (19), the cartridge (16), the bus (3), the disassemblers
(13), the boot ROM and the emitter (12), the DMA (13), the SIO (7), the timers/keypad/interrupts (12),
the demo
