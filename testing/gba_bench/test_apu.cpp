...
namespace
{
	...
	const u32 RegNR10 = 0x060, RegNR11 = 0x062, RegNR12 = 0x063, RegNR13 = 0x064, RegNR14 = 0x065;
	const u32 RegNR21 = 0x068, RegNR22 = 0x069, RegNR23 = 0x06C, RegNR24 = 0x06D;
	const u32 RegNR30 = 0x070, RegNR31 = 0x072, RegNR32 = 0x073, RegNR33 = 0x074, RegNR34 = 0x075;
	const u32 RegNR41 = 0x078, RegNR42 = 0x079, RegNR43 = 0x07C, RegNR44 = 0x07D;
	const u32 RegNR50 = 0x080, RegNR51 = 0x081, RegLow = 0x082, RegHigh = 0x083;
	const u32 RegCntX = 0x084, RegBiasLow = 0x088, RegBiasHigh = 0x089;
	const u32 RegWaveRam = 0x090, RegFifoA = 0x0A0, RegFifoB = 0x0A4;
	...
	struct Machine
	{
		...
		void Write(u32 offset, u8 value) { bus.apu.Write8(offset, value); }
		u8 Read(u32 offset) const { return bus.apu.Read8(offset, 0xA5); }
	...
	void PlayChannel1(Machine& m, int frequency, int duty, int volume)
	{
		...
		m.Write(RegNR11, (u8)(duty << 6));					// duty, length 0 means 64 steps
		m.Write(RegNR12, (u8)(volume << 4));				// volume, envelope step time 0
		m.Write(RegNR13, (u8)(frequency & 0xFF));
		m.Write(RegNR14, (u8)(0x80 | ((frequency >> 8) & 7)));
	}

	std::vector<s16> Left(const std::vector<s16>& frames)
	{
		std::vector<s16> out;
	...
	std::vector<s16> Right(const std::vector<s16>& frames)
	{
		std::vector<s16> out;
	...
	bool Silent(const std::vector<s16>& mono)
	{
	...
	int Peak(const std::vector<s16>& mono)
	{
		int peak = 0;
		for (size_t i = 0; i < mono.size(); i++)
		{
	...
	int MeasurePeriod(const std::vector<s16>& mono)
	{
	...
	void WriteWaveRam(Machine& m, const int* digits, int count)
	{
		for (int i = 0; i < 16; i++)
		{
			...
			m.Write(RegWaveRam + i, (u8)((high << 4) | low));
		}
	}
	...
	std::vector<int> NoiseLevels(int width, int period, int samples)
	{
		...
		{
			int target = (int)(((s64)k * SampleCycles) / period);
			while (clocks < target)
			{
				int carry = x & 1;
				x >>= 1;
...
GBA_TEST(Apu, RegisterReadWrite)
{
	...
	for (int i = 0; i < 16; i++)
		m.Write(RegWaveRam + i, (u8)(0x10 + i));
	for (int i = 0; i < 16; i++)
		GBA_CHECK_EQ(m.Read(RegWaveRam + i), (u8)(0x10 + i));
...
GBA_TEST(Apu, BusRegisterAccess)
{
	...
	bus.Write8(MemIo + RegNR13, (u8)(1792 & 0xFF));
...
GBA_TEST(Apu, SliceIndependence)
{
	...
	auto run = [](int slice) -> std::vector<s16>
	{
		Machine m;
		EnablePsg(m);
	...
	std::vector<s16> perSample = run(SampleCycles);
	std::vector<s16> perSlice = run(64);
...
GBA_TEST(Apu, DutyWaveform)
{
	...
	for (int duty = 0; duty < 4; duty++)
	{
		...
		std::vector<s16> mono = Left(m.Run(SampleCycles * 128));	// two periods
...
GBA_TEST(Apu, SweepChangesFrequency)
{
	...
	Machine m;
	EnablePsg(m);
	m.Write(RegNR10, (u8)(0x08 | (7 << 4) | 1));		// decrease, time 7, shift 1
	m.Write(RegNR11, 0x80);
	m.Write(RegNR12, 0xF0);
	m.Write(RegNR13, (u8)(1792 & 0xFF));
	m.Write(RegNR14, (u8)(0x80 | (1792 >> 8)));
	...
	m.Write(RegNR13, (u8)(1024 & 0xFF));
	m.Write(RegNR14, (u8)((1024 >> 8) & 7));

	std::vector<s16> afterPoke = Left(m.Run(Span));
	GBA_CHECK_EQ(MeasurePeriod(afterPoke), 256);		// 16 * (2048 - 1024) = 16384 cycles

	std::vector<s16> afterSweep = Left(m.Run(Span));
	GBA_CHECK_EQ(MeasurePeriod(afterSweep), 288);		// 16 * (2048 - 896) = 18432 cycles

	std::vector<s16> twice = Left(m.Run(Span));
...
GBA_TEST(Apu, SweepOverflowDisables)
{
	...
	// sweep"): 1500 + 750 overflows the 11 bit frequency, so the channel never starts.
	{
		Machine m;
		EnablePsg(m);
		m.Write(RegNR10, (u8)((1 << 4) | 1));			// increase, time 1, shift 1
		m.Write(RegNR11, 0x80);
		m.Write(RegNR12, 0xF0);
		m.Write(RegNR13, (u8)(1500 & 0xFF));
		m.Write(RegNR14, (u8)(0x80 | (1500 >> 8)));
	...
	// channel.
	{
		Machine m;
		EnablePsg(m);
		m.Write(RegNR10, (u8)((1 << 4) | 1));
		m.Write(RegNR11, 0x80);
		m.Write(RegNR12, 0xF0);
		m.Write(RegNR13, (u8)(1024 & 0xFF));
		m.Write(RegNR14, (u8)(0x80 | (1024 >> 8)));
...
GBA_TEST(Apu, SweepNegateQuirk)
{
	...
	Machine m;
	EnablePsg(m);
	m.Write(RegNR10, (u8)(0x08 | (1 << 4) | 1));		// decrease, time 1, shift 1
	m.Write(RegNR11, 0x80);
	m.Write(RegNR12, 0xF0);
	m.Write(RegNR13, (u8)(1792 & 0xFF));
	m.Write(RegNR14, (u8)(0x80 | (1792 >> 8)));
	...
	m.Write(RegNR10, (u8)((1 << 4) | 1));				// clear the direction bit
	GBA_CHECK_EQ(m.Read(RegCntX) & 0x01, 0x00);

	m.Write(RegNR10, (u8)(0x08 | (1 << 4) | 1));		// decrease again
	m.Write(RegNR14, (u8)(0x80 | (896 >> 8)));			// trigger
	GBA_CHECK_EQ(m.Read(RegCntX) & 0x01, 0x01);
...
GBA_TEST(Apu, Envelope)
{
	...
	up.Write(RegNR12, (u8)(0x08 | 0x01));				// volume 0, increase, step time 1
	up.Write(RegNR13, (u8)(1792 & 0xFF));
	up.Write(RegNR14, (u8)(0x80 | (1792 >> 8)));

	GBA_CHECK(Silent(Left(up.Run(EnvelopeStep))));
	std::vector<s16> raised = Left(up.Run(EnvelopeStep));
...
GBA_TEST(Apu, LengthCounter)
{
	...
	Machine m;
	EnablePsg(m);
	m.Write(RegNR11, (u8)(0x80 | 63));
	m.Write(RegNR12, 0xF0);
	m.Write(RegNR13, (u8)(1792 & 0xFF));
	m.Write(RegNR14, (u8)(0x80 | 0x40 | (1792 >> 8)));	// trigger, length enabled
	...
	auto run = [](int preTick) -> int
	{
		...
		m.Write(RegNR12, 0xF0);
		m.Write(RegNR13, (u8)(1792 & 0xFF));
		m.Write(RegNR14, (u8)(0x80 | 0x40 | (1792 >> 8)));
		m.Drain();

		for (int slices = 1; slices <= 200; slices++)
		{
			std::vector<s16> mono = Left(m.Run(LengthStep));
			if (Silent(mono))
				return slices;
		}
		return -1;
...
GBA_TEST(Apu, Channel2)
{
	...
	Machine m;
	EnablePsg(m);
	m.Write(RegNR21, (u8)(1 << 6));
	m.Write(RegNR22, 0xF0);
	m.Write(RegNR23, (u8)(1792 & 0xFF));
	m.Write(RegNR24, (u8)(0x80 | (1792 >> 8)));
	...
	std::vector<s16> rightOnly = Right(m.Run(SampleCycles * 64));
	...
	Machine env;
	EnablePsg(env);
	env.Write(RegNR21, (u8)(2 << 6));
	env.Write(RegNR22, 0x08 | 0x01);					// volume 0, increase, step time 1
	env.Write(RegNR23, (u8)(1792 & 0xFF));
	env.Write(RegNR24, (u8)(0x80 | (1792 >> 8)));
...
GBA_TEST(Apu, WavePlayback)
{
	...
	m.Write(RegNR33, (u8)(Frequency & 0xFF));
	m.Write(RegNR34, (u8)(0x80 | (Frequency >> 8)));

	std::vector<s16> pass = Left(m.Run(SampleCycles * 32));
	...
	std::vector<s16> again = Left(m.Run(SampleCycles * 32));
	...
	m.Write(RegNR34, (u8)(0x80 | (Frequency >> 8)));
	std::vector<s16> both = Left(m.Run(SampleCycles * 64));
	...
	m.Write(RegNR34, (u8)(0x80 | (Frequency >> 8)));
	std::vector<s16> bank1 = Left(m.Run(SampleCycles * 32));
	...
	struct VolumeCase { u8 nr32; int numerator; int denominator; };
...
GBA_TEST(Apu, WaveTimerSampling)
{
	...
	m.Write(RegNR34, (u8)(0x80 | 0x40 | (Frequency >> 8)));	// trigger + length flag = timer 1

	std::vector<s16> mono = Left(m.Run(SampleCycles * 64));
	GBA_CHECK_EQ((int)mono.size(), 64);
	...
	m.Write(RegNR34, (u8)(0x80 | (Frequency >> 8)));
	std::vector<s16> fast = Left(m.Run(SampleCycles * 8));
...
GBA_TEST(Apu, NoiseLfsr)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		m.Write(RegNR43, (u8)modes[i].nr43);
		m.Write(RegNR44, 0x80);							// trigger

		std::vector<s16> mono = Left(m.Run(SampleCycles * 40));
...
GBA_TEST(Apu, NoiseDividerTable)
{
	...
	for (int r = 0; r < 8; r++)
	{
		...
		m.Write(RegNR43, (u8)((3 << 4) | r));
		m.Write(RegNR44, 0x80);

		std::vector<s16> mono = Left(m.Run(SampleCycles * samples));
	...
	EnablePsg(m);
	m.Write(RegNR42, 0xF0);
	m.Write(RegNR43, (u8)((14 << 4) | 4));
	m.Write(RegNR44, 0x80);
	std::vector<s16> mono = Left(m.Run(SampleCycles * 16));
...
GBA_TEST(Apu, NoiseEnvelope)
{
	...
	Machine m;
	EnablePsg(m);
	m.Write(RegNR42, (u8)(0xF0 | 0x01));				// volume 15, decrease, step time 1
	m.Write(RegNR43, (u8)((1 << 4) | 4));
	m.Write(RegNR44, 0x80);
...
namespace
{
	...
	void SetupFifoA(Machine& m, int timer, bool right, bool left, int volume)
	{
		m.Write(RegCntX, 0x80);
		m.Write(RegNR50, 0x77);
		m.Write(RegLow, (u8)(0x02 | (volume << 2)));
		m.Write(RegHigh, (u8)((right ? 0x01 : 0) | (left ? 0x02 : 0) | (timer << 2)));
	}
	...
	void StartTimer(Machine& m, int index, u16 reload)
	{
...
GBA_TEST(Apu, FifoDmaRefill)
{
	...
	const u32 words[4] = { 0x04030201u, 0x08070605u, 0x0C0B0A09u, 0x100F0E0Du };
	...
	std::vector<s16> mono = Left(m.Run(2048));
	...
	std::vector<s16> more = Left(m.Run(2048 * 3));
...
GBA_TEST(Apu, FifoVolumeAndPanning)
{
	...
	const u32 words[4] = { 0x64646464u, 0x64646464u, 0x64646464u, 0x64646464u };

	auto run = [&](int volume, bool right, bool left) -> std::vector<s16>
	{
		Machine m;
	...
	std::vector<s16> full = run(1, true, true);
...
GBA_TEST(Apu, FifoTimerSelect)
{
	...
	std::vector<s16> idle = m.Run(2048 * 4);
	...
	std::vector<s16> playing = m.Run(4096 * 2);
...
GBA_TEST(Apu, MasterVolumeAndPanning)
{
	...
	auto run = [](u8 nr50, u8 nr51, u8 cntl) -> std::vector<s16>
	{
		Machine m;
		m.Write(RegCntX, 0x80);
	...
	std::vector<s16> full = run(0x77, 0xFF, 0x02);
	...
	std::vector<s16> split = run(0x07, 0xFF, 0x02);		// left 0 (mute), right 7
	GBA_CHECK(Silent(Left(split)));
	GBA_CHECK(!Silent(Right(split)));

	// NR51: bit 8 routes channel 1 to the right, bit 12 to the left.
	std::vector<s16> rightOnly = run(0x77, 0x01, 0x02);
	GBA_CHECK(Silent(Left(rightOnly)));
	GBA_CHECK(!Silent(Right(rightOnly)));

	std::vector<s16> leftOnly = run(0x77, 0x10, 0x02);
	GBA_CHECK(!Silent(Left(leftOnly)));
	GBA_CHECK(Silent(Right(leftOnly)));
