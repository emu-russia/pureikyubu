...
static void disa_goto(u32 addr)
{
...
static void con_disa_key(char ascii, int vkey, int ctrl)
{
    switch(vkey)
    {
        ...
        case VK_DOWN:
            ...
            {
                if(wind.x86line < (wind.x86lines - 2)) wind.x86line++;
            }
            break;
        case VK_PRIOR:
            if(wind.disamode == DISAMOD_PPC)
            ...
            {
                wind.x86line -= wind.disa_h - 1;
                if(wind.x86line < 0) wind.x86line = 0;
            }
            break;
        case VK_NEXT:
            if(wind.disamode == DISAMOD_PPC)
            ...
            {
                wind.x86line += wind.disa_h - 1;
                if(wind.x86line >= (wind.x86lines - 2)) wind.x86line = wind.x86lines - 2;
            }
            break;
        case VK_RETURN:
            if(ctrl & (LEFT_CTRL_PRESSED | RIGHT_CTRL_PRESSED))
            {
                if(wind.disamode == DISAMOD_PPC)
                {                       // show X86 result
                    con_disa_show_compile();
                    wind.disamode = DISAMOD_X86;
                    break;
                }
            }
            else
        ...
        case VK_ESCAPE:
            switch(wind.disamode)
            {
                case DISAMOD_PPC:       // browse back
                    disa_return();
                    break;
                case DISAMOD_X86:       // show PPC disasm
                    if(wind.x86dasm)
                    {
                        free(wind.x86dasm);
                        wind.x86dasm = NULL;
