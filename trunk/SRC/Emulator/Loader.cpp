...
u32 LoadDOL(char *dolname)
{
    ...
    MEMSwapArea((u32 *)&dh, sizeof(DolHeader));
...
u32 LoadDOLFromMemory(DolHeader *dol, u32 ofs)
{
    ...
    // swap DOL header
    MEMSwapArea((u32 *)dol, sizeof(DolHeader));
    ...
    for(i=0; i<DOL_NUM_TEXT; i++)
    {
        ...
        {
            u8 *addr = &RAM[dol->textAddress[i] & RAMMASK];
    ...
    for(i=0; i<DOL_NUM_DATA; i++)
    {
        ...
        {
            u8 *addr = &RAM[dol->dataAddress[i] & RAMMASK];
...
// we dont need to translate address, because DOL loading goes
// under control of DolphinOS, so just use simple translation mask.
unsigned long LoadELF(char *elfname)
{
...
{
    u32 org = GetConfigInt(USER_BINORG, USER_BINORG_DEFAULT);

    // binary file loading address is above RAM
    if(org >= RAMSIZE) return 0;

    // get file size
    s32 fsize = FileSize(binname);
...
BOOL LoadPatch(char * patchname, BOOL add)
{
    ...
    // count patchnum
    s32 patchNum = FileSize(patchname) / sizeof(Patch);
...
void ApplyPatches(BOOL load, s32 a, s32 b)
{
    // allowed ?
...
static void DoLoadFile(char *filename)
{
    ...
    // show boot time
    u32 e_time = GetTickCount();
    ldat.boottime = (f32)(e_time - s_time) / 1000.0f;
