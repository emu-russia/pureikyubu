...
dspjit_x86.cpp is the 32-bit sibling of this file.
...
#elif !defined(DSP_JIT_X86)

// Everything that does not build a recompiler - a non-x86 host, or any host with
// DSP_JIT_DISABLED - keeps the interpreter: this stub is a no-op that reports itself as
// unsupported, so Update() runs the interpreter. It is the module that owns the fallback
// whenever the 32-bit one is not in the build. DumpBlock is part of the stub as well -
// the debug path calls it on every host, and leaving it out is what kept the 32-bit
// build from linking (issue #417).
