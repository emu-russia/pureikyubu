...
namespace Flipper
{
	...
	// reset VI timing
	void VideoInterface::vi_set_timing()
	{
		...
		if ((Core->GetTicks() - vi.vtime) >= (vi.one_frame / vi.vcount))
		{
			...
			if (vi.pos.vcount == vi.int0.vcount)
			{
				...
				if (vi.int0.enabled)
				{
					...
					HW->pi->PIAssertInt(PI_INTERRUPT_VI);
				}
			}
	...
	void VideoInterface::VIRegRead(uint32_t addr, uint32_t* reg, void* context)
	{
		VideoInterface* vi = (VideoInterface*)context;
		switch (addr & 0x7f)
		{
			case VI_DISP_CR:
				*reg = vi->vi.disp_cr;
			...
			case VI_BFBL:
				*reg = vi->vi.bfbl >> 16;
				break;
			case VI_BFBL + 2:
				*reg = (uint16_t)vi->vi.bfbl;
				break;
			case VI_DISP_POS:
				*reg = vi->vi.pos.val >> 16;
				break;
			case VI_DISP_POS + 2:
				*reg = (uint16_t)vi->vi.pos.val;
				break;
			case VI_INT0:
				*reg = vi->vi.int0.val >> 16;
				break;
			case VI_INT0 + 2:
				*reg = (uint16_t)vi->vi.int0.val;
				break;
			default:
				*reg = 0;
				break;
	...
	void VideoInterface::VIRegWrite(uint32_t addr, uint32_t data, void* context)
	{
		VideoInterface* vi = (VideoInterface*)context;
		if (vi->vi.log) {
		...
		switch (addr & 0x7f)
		{
			...
			case VI_TFBL:
				...
				if (vi->vi.log)
				{
					Report(Channel::VI, "TFBL set to %08X (xof=%i)\n", vi->vi.tfbl, (vi->vi.tfbl >> 24) & 0xf);
				}
				vi->vi.tfbl &= 0xffffff;
				vi->vi.xfbbuf = (uint8_t*)HW->mem->MIGetMemoryPointerForVI(vi->vi.tfbl);
				break;
			case VI_TFBL + 2:
				vi->vi.tfbl &= 0xffff0000;
				vi->vi.tfbl |= (uint16_t)data;
				if (vi->vi.log)
				{
					Report(Channel::VI, "TFBL set to %08X (xof=%i)\n", vi->vi.tfbl, (vi->vi.tfbl >> 24) & 0xf);
				}
			...
			case VI_BFBL:
				...
				break;
			case VI_BFBL + 2:
				vi->vi.bfbl &= 0xffff0000;
				vi->vi.bfbl |= (uint16_t)data;
				vi->vi.bfbl &= 0xffffff;
				if (vi->vi.log)
				{
					Report(Channel::VI, "BFBL set to %08X\n", vi->vi.bfbl);
				...
				break;
			case VI_INT0:
				vi->vi.int0.val &= 0x0000ffff;
				vi->vi.int0.val |= data << 16;
				if ((vi->vi.int0.val & VI_INT_INT) == 0)
				{
					HW->pi->PIClearInt(PI_INTERRUPT_VI);
				}
				break;
			case VI_INT0 + 2:
				vi->vi.int0.val &= 0xffff0000;
				vi->vi.int0.val |= (uint16_t)data;
				break;
			default:
				break;
		}
	...
	void VideoInterface::VIGunTrigger(int num)
	{
		switch (num & 1) {
			case 0:
