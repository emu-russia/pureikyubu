...
uint32_t Jit::CompileBlock(uint32_t pc, uint32_t pa, uint32_t& instrCount)
{
	...
	e.sub_r32_r32(T1, T0);
	e.mov_m32_r(X64::RSP, DeadlineSlot, T1);
	...
	auto emitBackEdge = [&](uint32_t targetPc)
	{
		...
		e.mov_r32_m(T0, X64::RSP, LoopTicksSlot);
		e.alu_r32_r32(X64::AluAdd, T0, RegCount);
		e.mov_r32_m(T1, X64::RSP, DeadlineSlot);
		e.alu_r32_r32(X64::AluCmp, T0, T1);
		size_t notDue = e.jcc_rel32(X64::CcB);
	...
	while (count < MaxBlockInstrs && !endBlock)
	{
		...
		case Instruction::lwz: case Instruction::lwzu: case Instruction::lwzx: case Instruction::lwzux:
		{
			...
			if (count > 1)
			{
				e.mov_r32_r32(T0, X64::RAX);
				e.and_r32_imm(T0, 0xffff'0000);
				e.alu_r32_imm(X64::AluCmp, T0, 0xcc00'0000);
				size_t notDevice = e.jcc_rel32(X64::CcNE);
				...
				e.alu_r32_imm(X64::AluSub, RegCount, 1);
...
void Jit::RunInner()
{
	...
	int64_t tbBefore = core->regs.tb.sval;
	{
	...
	case JitExitKind::TakenBranch:
		...
		// branch. Charging it only one put the two engines a tick apart at tb 4 548 031, on a
		// block that looped once and then branched out of the loop.
