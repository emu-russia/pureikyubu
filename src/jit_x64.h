...
Shared x86-64 encoder and host calling-convention layout.

The Gekko basic-block recompiler (gekkojit.cpp / gekkojit_ps.cpp) and the DSPcore
recompiler (dspjit.cpp) both emit x86-64 machine code through the same Emitter, so it
lives here, together with the argument registers of the host ABI (Win64 and SysV).
The parts that are specific to one emulated core - the host register roles, the stack
frame size and the offsets into that core's register file - live next to that core
(see gekkojit_layout.h and the layout section of dspjit.h).
...
public:
	...
	// mistake invisible on Linux; in the ABI regression build (GEKKO_ or DSP_)
	// this poisons exactly that area before every helper call, so the corruption is
	// reproducible there too. R11 is free: call_abs loads it with the target right
	// afterwards.
	void poison_shadow_space()
	{
#if defined(GEKKO_JIT_TEST_WIN64_SHADOW) || defined(DSP_JIT_TEST_WIN64_SHADOW)
