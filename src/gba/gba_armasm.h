...
namespace GBA
{
	namespace ArmAsm
	{
		...
		public:
			...
			void Mov(int rd, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);

			/// <summary>MOV{S} rd, rm (optionally shifted).</summary>
			void MovReg(int rd, int rm, Cond c = Cond::AL, bool setFlags = false,
				Shift shift = Shift::LSL, uint32_t amount = 0);

			void Add(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void AddReg(int rd, int rn, int rm, Cond c = Cond::AL, bool setFlags = false);
			void Sub(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void SubReg(int rd, int rn, int rm, Cond c = Cond::AL, bool setFlags = false);
			void Rsb(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void And(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void AndReg(int rd, int rn, int rm, Cond c = Cond::AL, bool setFlags = false);
			void Orr(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void OrrReg(int rd, int rn, int rm, Cond c = Cond::AL, bool setFlags = false);
			void Eor(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void EorReg(int rd, int rn, int rm, Cond c = Cond::AL, bool setFlags = false);
			void Bic(int rd, int rn, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void Mvn(int rd, uint32_t imm, Cond c = Cond::AL, bool setFlags = false);
			void Cmp(int rn, uint32_t imm, Cond c = Cond::AL);
			void CmpReg(int rn, int rm, Cond c = Cond::AL, Shift shift = Shift::LSL, uint32_t amount = 0);
			void Tst(int rn, uint32_t imm, Cond c = Cond::AL);
			...
			void ShiftReg(int rd, int rm, Shift shift, uint32_t amount, Cond c = Cond::AL, bool setFlags = false);
			...
			void Ldmia(int rn, uint32_t regList, Cond c = Cond::AL, bool writeBack = true);
			void Stmia(int rn, uint32_t regList, Cond c = Cond::AL, bool writeBack = true);
			void Push(uint32_t regList) { Stmia(13, regList, Cond::AL, true); }
			void Pop(uint32_t regList) { Ldmia(13, regList, Cond::AL, true); }
			...
			void ThumbMov(int rd, uint32_t imm8);
			void ThumbMovReg(int rd, int rm);
			void ThumbAdd(int rd, int rn, int rm);
			void ThumbAddImm(int rd, uint32_t imm8);
			void ThumbSubImm(int rd, uint32_t imm8);
			void ThumbCmpImm(int rd, uint32_t imm8);
			void ThumbLsl(int rd, int rm, uint32_t amount);
			void ThumbLsr(int rd, int rm, uint32_t amount);
			void ThumbAsr(int rd, int rm, uint32_t amount);
			void ThumbAnd(int rd, int rm);
			void ThumbOrr(int rd, int rm);
			void ThumbEor(int rd, int rm);
			void ThumbMvn(int rd, int rm);
			void ThumbLdrImm(int rd, int rn, uint32_t offset);
			void ThumbStrImm(int rd, int rn, uint32_t offset);
			void ThumbLdrhImm(int rd, int rn, uint32_t offset);
			void ThumbStrhImm(int rd, int rn, uint32_t offset);
			void ThumbLdrbImm(int rd, int rn, uint32_t offset);
			void ThumbStrbImm(int rd, int rn, uint32_t offset);
			void ThumbPush(uint32_t regList);
			void ThumbPop(uint32_t regList);
			void ThumbB(const std::string& label, Cond c = Cond::AL);
			void ThumbBx(int rm);
			void ThumbSwi(uint32_t comment);
			...
			/// </summary>
			std::vector<uint8_t> TakeImage(uint32_t size = 0);
		...
		private:
			...
			struct Fixup
			{
				uint32_t address;			// where the branch instruction is
				std::string label;
				bool thumb;
				bool link;
				bool conditional;
			...
			void Emit(uint32_t word, const std::string& text);
			void Emit16(uint16_t halfword, const std::string& text);
			uint32_t EncodeArmImmediate(uint32_t value, bool& ok) const;
